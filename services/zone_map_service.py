...
# 대전 및 주요 재개발/정비사업 구역 좌표 데이터베이스 (0ms 초고속 해결)
KNOWN_ZONES = {
    "도마변동5구역": {"lat": 36.3168, "lon": 127.3785, "title": "도마·변동 5구역 재정비촉진지구", "addr": "대전 서구 변동 일원"},
    "도마변동1구역": {"lat": 36.3262, "lon": 127.3770, "title": "도마·변동 1구역 재정비촉진지구", "addr": "대전 서구 가장동 일원"},
    "도마변동3구역": {"lat": 36.3210, "lon": 127.3795, "title": "도마·변동 3구역 재정비촉진지구", "addr": "대전 서구 변동 일원"},
    "도마변동4구역": {"lat": 36.3225, "lon": 127.3745, "title": "도마·변동 4구역 재정비촉진지구", "addr": "대전 서구 변동 일원"},
    "도마변동6구역": {"lat": 36.3155, "lon": 127.3820, "title": "도마·변동 6구역 재정비촉진지구", "addr": "대전 서구 도마동 일원"},
    "도마변동8구역": {"lat": 36.3140, "lon": 127.3845, "title": "도마·변동 8구역 (도마e편한세상포레나)", "addr": "대전 서구 도마동 일원"},
    "도마변동9구역": {"lat": 36.3180, "lon": 127.3870, "title": "도마·변동 9구역 재정비촉진지구", "addr": "대전 서구 도마동 일원"},
    "도마변동11구역": {"lat": 36.3120, "lon": 127.3800, "title": "도마·변동 11구역 (호반써밋)", "addr": "대전 서구 도마동 일원"},
    "도마변동12구역": {"lat": 36.3170, "lon": 127.3720, "title": "도마·변동 12구역 재정비촉진지구", "addr": "대전 서구 도마동 일원"},
    "도마변동13구역": {"lat": 36.3135, "lon": 127.3745, "title": "도마·변동 13구역 재정비촉진지구", "addr": "대전 서구 도마동 일원"},
    "탄방1구역": {"lat": 36.3450, "lon": 127.3910, "title": "탄방동 1구역 (둔산자이아이파크)", "addr": "대전 서구 탄방동 일원"},
    "숭어리샘": {"lat": 36.3430, "lon": 127.3870, "title": "탄방동 숭어리샘 재건축", "addr": "대전 서구 탄방동 일원"},
    "용문123구역": {"lat": 36.3380, "lon": 127.3980, "title": "용문 1·2·3구역 (둔산더샵엘리프)", "addr": "대전 서구 용문동 일원"},
    "선화구역": {"lat": 36.3310, "lon": 127.4200, "title": "선화 재정비촉진구역", "addr": "대전 중구 선화동 일원"},
    "대흥2구역": {"lat": 36.3220, "lon": 127.4260, "title": "대흥 2구역 주택재개발", "addr": "대전 중구 대흥동 일원"}
}


def _normalize_name(name: str) -> str:
    """구역명 비교를 위한 정규화 (공백/특수기호 제거)"""
    return re.sub(r"[^\w가-힣a-zA-Z0-9]", "", name)
...
def _draw_zone_annotations(p: QPainter, width: int, height: int, display_title: str, office_name: str):
    ...
    # 1. 정비구역 점선 테두리 및 틴트
    p.setPen(QPen(QColor(239, 68, 68, 240), 3, Qt.PenStyle.DashLine))
    p.setBrush(QColor(239, 68, 68, 38))
    zone_rect = QRectF(width // 2 - 140, height // 2 - 100, 280, 200)
    ...
    # 3. 핀 라벨 배너
    badge_rect = QRectF(width // 2 - 140, height // 2 - 62, 280, 36)
    ...
    p.setPen(Qt.PenStyle.NoPen)
    p.drawRect(0, 0, width, 48)

    p.setPen(QColor(255, 255, 255))
    p.setFont(QFont(FONT_FAMILY, 13, QFont.Weight.Bold))
    p.drawText(QRectF(20, 0, width - 40, 48), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, f"🗺️ 국토교통부 토지이용계획 및 도시정비구역 참조 | {display_title}")

    # 5. 하단 푸터 및 범례
    p.setBrush(QColor(15, 23, 42, 230))
    p.drawRect(0, height - 36, width, 36)

    p.setPen(QColor(148, 163, 184))
    p.setFont(QFont(FONT_FAMILY, 10, QFont.Weight.Normal))
    p.drawText(QRectF(20, height - 36, 450, 36), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, "■ 빨간점선: 재개발 정비구역 예정지  ■ 용도: 제2종일반주거지역")
    p.drawText(QRectF(width - 430, height - 36, 410, 36), Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter, f"🏢 {office_name} | 블로그 브리핑용 구역 위치도")


def generate_zone_map_image(zone_or_addr: str, office_name: str = "신우 공인중개사사무소") -> QImage:
    """
    입력된 구역명/주소를 기반으로 16:9 고해상도(960x540) 정비구역 위치도 이미지 생성
    """
    width = 960
    height = 540
    zoom = 16
