...
def _build_news_prompt(topic: str, cfg: dict) -> str:
    ...
    return (
        ...
        f"### [🔍 네이버 블로그 포스팅 필수 작성 및 저작권 준수 지침]:\n"
        f"1. **실시간 수집 팩트 적극 인용**: 위에 실시간으로 수집된 최신 언론 보도의 핵심 내용, 발표 시점, 통계 수치를 본문에 자연스럽게 반영하여 글의 신뢰성을 극대화하세요.\n"
        f"2. **🚨 언론사 저작권 보호 (필수)**: 기사 본문을 그대로 복사하거나 일부 단어만 바꾸어 전재하지 마세요. 객관적 사실(Fact)과 수치만 추출하고, 글의 80% 이상은 공인중개사의 전문 시각에서 분석한 시장 영향, 매수/매도/임차인별 맞춤 가이드, 실무 팁 등 독창적인 새 창작물로 작성해야 합니다.\n"
        f"3. **정확한 출처 표기**: 본문 서두에 '최근 연합뉴스 등 언론 보도에 따르면'과 같이 출처를 자연스럽게 명시하세요.\n"
        f"4. **작성 범위 및 최신성 필터**: {period_desc}.\n"
        f"5. **오래된 기사(1~2년 전 과거 자료) 엄격 배제**: 1~2년 전({past_years_str})의 지난 기사나 이미 개정된 구형 정책을 현재의 사실처럼 인용하는 것은 절대 불가합니다.\n"
        f"6. **본문 구성 및 시사점**: 최신 팩트와 현재 시장 상황, 실제 부동산 거래 및 매수/임차인에게 미치는 실질적인 영향과 대응 전략을 공인중개사의 신뢰감 있는 시각으로 알기 쉽게 브리핑해주세요."
    )
