...
def _get_pyinstaller_cmd(base_dir: str, app_script: str, mode: str) -> list:
    """PyInstaller 실행 명령어 리스트 조립"""
    spec_file = os.path.join(base_dir, "공인중개사_블로그_생성기.spec")
    if os.path.exists(spec_file) and mode == "onedir":
        return [sys.executable, "-m", "PyInstaller", "--noconfirm", spec_file]
...
def _handle_build_success(base_dir: str, mode: str):
    """빌드 성공 후 결과 안내 출력 및 단일 exe 복사"""
...
def build(mode="onedir"):
    ...
    cmd = _get_pyinstaller_cmd(base_dir, app_script, mode)

    print("Running command:", " ".join(cmd))
    result = subprocess.run(cmd)

    if result.returncode == 0:
        _handle_build_success(base_dir, mode)
    else:
        print("\n[Error] Build failed.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and ("--onefile" in sys.argv or "onefile" in sys.argv):
        target_mode = "onefile"
