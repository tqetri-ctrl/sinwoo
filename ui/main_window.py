...
- 초등학생도 쉽게 사용할 수 있는 1-2-3-4 단계식 직관적 워크플로우
- 단정하고 깔끔한 디자인 (Clean & Neat)
...
class BlogGenerationThread(QThread):
    ...
    def __init__(self, service: GeminiBlogService, mode: str, topic: str, file_path: str, tone_key: str, config: dict):
        super().__init__()
...
class SettingsDialog(QDialog):
    """초보자도 알기 쉬운 간편 설정 창 (API 키 & 중개사무소 정보)"""
...
class MainWindow(QMainWindow):
    ...
    def init_window(self):
        self.setWindowTitle("공인중개사 네이버 블로그 글 생성기 (AI 부동산 브리핑)")
    ...
    def init_ui(self):
        ...
        # 본문 스플리터 (좌: 입력 및 옵션 45%, 우: 결과 및 미리보기 55%)
    ...
    def create_header(self) -> QWidget:
        ...
        lbl_subtitle = QLabel("주제 또는 파일만 넣으면, 최신 기사를 검색하고 이모티콘을 곁들인 블로그 글을 뚝딱 만듭니다.")
    ...
    def create_left_input_panel(self) -> QWidget:
        ...
        title1 = QLabel("글감 넣기 (주제 또는 첨부파일)")
        ...
        # 탭 위젯 (인터넷 기사 검색 모드 vs 내 파일 업로드 모드)
        ...
        self.edit_news_topic.setFixedHeight(95)
        tab_news_layout.addWidget(self.edit_news_topic)
        self.input_tab.addTab(tab_news, "📰 인터넷 기사 찾아서 쓰기")
        ...
        lbl_file_guide = QLabel("📁 보도자료, 분양 공고문, HWP, PDF, 워드, 사진 파일을 선택해주세요.")
        ...
        self.input_tab.addTab(tab_file, "📁 내 파일 넣어서 쓰기")
        ...
        self.radio_neighbor = QRadioButton("☕ 다정하고 친절한 이웃 말투 (따뜻한 해요체, 편안한 설명)")
        self.radio_expert = QRadioButton("🏢 신뢰감 넘치는 부동산 전문가 말투 (명확한 수치/정책 브리핑)")
        self.radio_coach = QRadioButton("📈 스마트한 부동산 투자 코칭 말투 (매수/투자 실전 인사이트)")
    ...
    def create_right_result_panel(self) -> QWidget:
        ...
        <div style='text-align: center; color: #94A3B8; padding: 100px 20px; font-size: 15px;'>
            <h3>👈 좌측에서 주제나 파일을 넣고<br>[✨ 네이버 블로그 글 만들기] 버튼을 눌러주세요!</h3>
        ...
        self.edit_tags.setPlaceholderText("#부동산 #공인중개사 #부동산소식")
    ...
    def on_select_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "블로그 글을 작성할 첨부 파일 선택",
            "",
            "모든 지원 파일 (*.pdf *.docx *.hwpx *.hwp *.txt *.png *.jpg *.jpeg);;PDF 파일 (*.pdf);;한글 문서 (*.hwpx *.hwp);;워드 문서 (*.docx);;이미지 (*.png *.jpg *.jpeg);;텍스트 (*.txt)"
        )
        if file_path:
    ...
    def on_start_generate(self):
        ...
        if current_tab_idx == 0:
            ...
            if not file_path:
                QMessageBox.warning(self, "파일 선택 필요", "분석할 첨부 파일(PDF, HWP, DOCX, 이미지 등)을 먼저 선택해주세요!")
                return
        ...
        if mode == "news":
            self.lbl_loading_status.setText("🔍 최신 인터넷 기사를 검색하고 전문 블로그 글을 작성하고 있습니다...")
        else:
            self.lbl_loading_status.setText("📄 첨부파일 내용을 정밀 분석하여 블로그 글을 작성하고 있습니다...")
    ...
    def on_copy_for_naver(self):
        ...
        body_html = markdown.markdown(body_text, extensions=['extra', 'nl2br'])
